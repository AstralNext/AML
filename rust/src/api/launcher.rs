...
#[derive(Clone, Debug)]
pub struct DeviceCodeLoginBeginDto {
    pub login_id: String,
...
pub async fn begin_device_code_login() -> Result<DeviceCodeLoginBeginDto, String> {
    launcher::auth::begin_device_code_login()
        .await
        .map(|l| DeviceCodeLoginBeginDto {
            login_id: l.login_id,
            user_code: l.user_code,
            verification_uri: l.verification_uri,
            interval: l.interval,
            expires_in: l.expires_in,
            message: l.message,
        })
        .map_err(|e| {
            eprintln!("[AML] begin_device_code_login: {e:#}");
            e.to_string()
        })
}

#[derive(Clone, Debug)]
pub struct DeviceCodePollResultDto {
...
impl DeviceCodePollResultDto {
    const STATUS_PENDING: &'static str = "pending";
    const STATUS_SUCCESS: &'static str = "success";
    const STATUS_EXPIRED: &'static str = "expired";
    const STATUS_DECLINED: &'static str = "declined";
    const STATUS_SLOW_DOWN: &'static str = "slow_down";
...
pub async fn poll_device_code(
    login_id: String,
) -> Result<DeviceCodePollResultDto, String> {
    launcher::auth::poll_device_code(&login_id)
        .await
        .map(|r| match r {
            launcher::auth::DeviceCodePollResult::Pending => {
                DeviceCodePollResultDto::empty(DeviceCodePollResultDto::STATUS_PENDING)
            }
            launcher::auth::DeviceCodePollResult::Expired => {
                DeviceCodePollResultDto::empty(DeviceCodePollResultDto::STATUS_EXPIRED)
            }
            launcher::auth::DeviceCodePollResult::Declined => {
                DeviceCodePollResultDto::empty(DeviceCodePollResultDto::STATUS_DECLINED)
            }
            launcher::auth::DeviceCodePollResult::SlowDown => {
                DeviceCodePollResultDto::empty(DeviceCodePollResultDto::STATUS_SLOW_DOWN)
            }
        ...
        .map_err(|e| {
            eprintln!("[AML] poll_device_code: {e:#}");
            e.to_string()
        })
}
