...
pub struct CdnSettings {
    ...
    #[serde(default = "default_true")]
    pub bmclapi: bool,
    /// Parallel Range connections per large file. Each part stays ≥ 2 MiB.
...
pub const DOWNLOAD_THREADS_MIN: u32 = 1;
...
/// User-configured max Range connections (1 ..= CPU logical threads).
pub fn download_max_parts() -> usize {
    let cap = cpu_thread_count();
    cdn_settings()
        .download_threads
        .clamp(DOWNLOAD_THREADS_MIN, cap) as usize
}
...
/// CurseForge API key，构建时通过编译期环境变量 `AML_CURSEFORGE_API_KEY`
/// 注入（CI 传 GitHub Secret）。本地开发未注入时为空，官方 API 401 后
/// 自动降级 MCIM 镜像。运行时也可用同名环境变量覆盖。
pub const CURSEFORGE_API_KEY_DEFAULT: &str = match option_env!("AML_CURSEFORGE_API_KEY") {
    Some(key) => key,
    None => "",
};
