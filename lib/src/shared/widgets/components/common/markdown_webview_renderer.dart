...
/// WebView2 渲染的 Markdown 正文（Windows）。
/// 环境引导、高度测量桥接与外链/图片交互都在本文件内。
class MarkdownWebViewContent extends StatefulWidget {
  const MarkdownWebViewContent({
    super.key,
    required this.data,
  ...
  @override
  State<MarkdownWebViewContent> createState() =>
      _MarkdownWebViewContentState();
}

class _MarkdownWebViewContentState extends State<MarkdownWebViewContent> {
  ...
  @override
  void didUpdateWidget(covariant MarkdownWebViewContent oldWidget) {
...
/// 纯 [Html] 渲染回退（WebView 不可用/损坏或正文过长时）。
class MarkdownHtmlFallback extends StatelessWidget {
  const MarkdownHtmlFallback({
    super.key,
    required this.data,
    required this.tokens,
  });
