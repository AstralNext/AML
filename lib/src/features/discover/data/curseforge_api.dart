...
class CurseForgeApiService {
  ...
  /// CurseForge API key，构建时通过 `--dart-define=CURSEFORGE_API_KEY=...`
  /// 注入（CI 传 GitHub Secret）。本地开发未注入时为空，官方 API 返回
  /// 401 后会自动降级到 MCIM 镜像。不要在日志中打印此值。
  static const _apiKey = String.fromEnvironment('CURSEFORGE_API_KEY');
