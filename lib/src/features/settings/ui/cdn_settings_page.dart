      ...
      child: Watch((context) {
        ...
        return ListView(
          children: [
            ...
            const SizedBox(height: 8),
            Text(
              '大于 10MB 的文件会按 Range 分片并行拉取。每片至少 2MB。线程上限等于本机 CPU 逻辑线程数。',
              style: TextStyle(
                fontSize: 14,
                color: tokens.colorBase.withValues(alpha: 0.7),
                height: 1.4,
            ...
            _card(
              ...
              child: Builder(
                builder: (context) {
                  final cpuMax = cpuThreadCount();
                  final threads =
                      ui.downloadThreads.watch(context).clamp(1, cpuMax);
                  return Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                            ...
                            children: [
                              Expanded(
                                child: Slider(
                                  ...
                                  min: minDownloadThreads.toDouble(),
                                  max: cpuMax.toDouble(),
                                  divisions: cpuMax - minDownloadThreads,
                                  activeColor: tokens.colorBrand,
                                  label: '$threads',
                                  onChanged: (value) =>
                                      ui.setDownloadThreads(value.round()),
                                ),
                              ),
                              SizedBox(
                                width: 72,
                        ...
                        Text(
                          threads == 1
                              ? '单连接下载'
                              : '最多 $threads 路 / 本机 $cpuMax 线程 · 每片 ≥ 2MB',
                          style: TextStyle(
                            fontSize: 12,
                            color: tokens.colorBase.withValues(alpha: 0.7),
                          ),
